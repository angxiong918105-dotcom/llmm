...
export default {
  async fetch(request, env) {
    ...
    */
    ...
    const systemPrompt =
      "You are an expert design and product evaluation assistant. " +
      "Assess short written responses about measurement, uncertainty, and design judgment. " +
      "Evaluate fairly based on conceptual understanding, reasoning quality, and practical relevance. " +
      "Be concise, constructive, and non-punitive. " +
      "Do not assume missing intent; judge only what is written. " +
      "Return ONLY valid JSON (no markdown, no extra text). " +
      'The verdict must be exactly one of: "Correct", "Not quite right", or "Incorrect".';
    ...
    const userPrompt =
      ...
      `Learner response:\n${responseText}\n\n` +
      "Evaluation task:\n" +
      "Assess whether the learner explains why additional measurement may fail to eliminate uncertainty, " +
      "and whether they propose at least one practical way to balance quantitative metrics with design judgment.\n\n" +
      "Guidelines:\n" +
      "- Look for recognition of limits of metrics (e.g., proxies, noise, misalignment, gaming/Goodhart’s law, overfitting to what is measurable).\n" +
      "- The response should address BOTH parts: (1) why measurement may not create certainty, and (2) one practical balancing approach.\n" +
      "- The proposed approach should be concrete and actionable (e.g., run a small experiment with clear success criteria, pair metrics with qualitative research, decision memo with assumptions, prototype + usability test, triangulation).\n" +
      "- Minor writing issues are acceptable if the ideas are clear.\n\n" +
...
function isAllowedOrigin(origin) {
  ...
  if (origin === "https://angxiong918105-dotcom.github.io") return origin;
